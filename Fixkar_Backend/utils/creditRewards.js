...
export const rewardProfessionalMilestones = async ({ professionalId, walletId, bookingId, session }) => {
  ...
  for (const claim of claims) {
    ...
    for (const milestone of eligible) {
      ...
      await CreditTransaction.create([{ walletId, professionalId, type: "EARNED", source: "professional_milestone", credits, referenceId: bookingId, referenceModel: "Booking", description: `${offer.offerTitle} - ${milestone.title || `${target} completed bookings`} reward`, metadata: { offerId: offer._id, couponCode: offer.couponCode, targetBookings: target, rank: milestone.badge } }], { session });
      ...
      existingKeys.add(rewardKey); existingMilestones.add(target);
      rewards.push({ offerId: offer._id, couponCode: offer.couponCode, targetBookings: target, credits, rank: milestone.badge });
    }
  }
