...
const getRankForBookings = (completedBookings) => completedBookings >= 10 ? "DIAMOND" : completedBookings >= 5 ? "SILVER" : completedBookings >= 1 ? "BRONZE" : "NEWCOMER";
...
export const rewardProfessionalMilestones = async ({ professionalId, walletId, bookingId, session }) => {
  ...
  const claims = await OfferClaim.find({ userId: professional.userId, status: { $in: ["claimed", "redeemed"]] })
    .populate({ path: "offerId", match: { benefitType: "PROFESSIONAL_REWARD", rewardTrigger: { $in: ["FIRST_COMPLETED_BOOKING", "BOOKING_COUNT_MILESTONE"] }, isActive: true } })
    .session(session);
  const rewards = [];
  ...
  for (const claim of claims) {
    ...
    const milestones = Array.isArray(offer.milestones) && offer.milestones.length
      ? offer.milestones
      : [{ bookingCount: 1, rewardCredits: Number(offer.rewardValue || 0), badge: "BRONZE", title: "First completed booking" }];
    const baseline = Number(claim.startingCompletedBookings || 0);
    const eligible = milestones
      .filter(m => Number(m.bookingCount) > baseline && Number(m.bookingCount) <= completedBookings && Number(m.rewardCredits) > 0)
      .sort((a, b) => Number(a.bookingCount) - Number(b.bookingCount));

    for (const milestone of eligible) {
      const target = Number(milestone.bookingCount);
      const rewardKey = `${offer._id}:${target}`;
      if (existingKeys.has(rewardKey) || claim.rewardedMilestones?.includes(target)) continue;
      ...
      if (alreadyRewarded) {
        ...
        existingMilestones.add(target);
        continue;
      }

      await Wallet.findByIdAndUpdate(walletId, { $inc: { "credits.balance": credits, "credits.lifetimeEarned": credits } }, { session });
      await CreditTransaction.create([{
        walletId, professionalId, type: "EARNED", source: rewardSource, credits, referenceId: bookingId, referenceModel: "Booking",
        description: `${offer.offerTitle} - ${milestone.title || `${target} completed bookings`} reward`,
        metadata: { offerId: offer._id, couponCode: offer.couponCode, targetBookings: target, rank: milestone.badge },
      }], { session });
      await OfferUsage.create([{
        offerId: offer._id, userId: professional.userId, bookingId, couponCode: offer.couponCode, rewardCredits: credits,
        milestoneTarget: target, milestoneRank: milestone.badge, paymentMode: "REWARD", status: "used", offerSnapshot: { title: offer.offerTitle },
      }], { session });
  ...
  await Professional.findByIdAndUpdate(professionalId, {
    $set: {
      ...
      "achievements.unlockedMilestones": [...existingMilestones],
      "achievements.unlockedRewardKeys": [...existingKeys],
    },
  }, { session });
...
export const rewardCompletedBookingCredits = async ({ booking, walletId, professionalEarnings, session }) => {
  ...
  if (bookingCredits > 0) {
    ...
    if (!existingTransaction) {
      await CreditTransaction.create([{
        walletId, professionalId, type: "EARNED", source: "booking_completion", credits: bookingCredits, referenceId: bookingId, referenceModel: "Booking",
        description: "Credits earned for completing a booking", metadata: { professionalEarnings: Number(professionalEarnings) || 0 },
      }], session ? { session } : {});
