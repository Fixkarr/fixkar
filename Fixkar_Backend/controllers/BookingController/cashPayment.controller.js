...
export const confirmCashPayment = async (req, res) => {
  try {
    ...
    if (await Payment.findOne({ bookingId, status: "paid" })) return res.status(400).json({ message: "Payment already confirmed" });
    ...
    booking.status = "completed"; booking.currentPaymentId = payment._id; booking.completedAt = new Date(); await booking.save();
    ...
    try { await rewardCompletedBookingCredits({ booking, walletId: wallet._id, professionalEarnings: professionalAmount }); } catch (creditError) { console.error("Unable to award booking credits:", creditError); }

    await PlatformTransaction.create({ bookingId: booking._id, paymentId: payment._id, professionalId: booking.professionalId._id, paymentMode: "CASH", grossAmount: fullAmount, customerPaidAmount: fullAmount - discountAmount, discountAmount, professionalAmount, commission, profitOrLoss: commission - discountAmount });
    ...
    if (booking.offerLocked && booking.offerId) {
      await OfferUsage.create({ offerId: booking.offerId, userId: booking.customerId.userId._id, bookingId: booking._id, couponCode: booking.offerCode, offerSnapshot: booking.offerSnapshot, discountAmount, paymentMode: "CASH" });
  ...
  } catch (error) { console.error(error); res.status(500).json({ message: "Cash payment confirmation failed" }); }
};
