...
export const claimCouponController = async (req, res) => {
  try {
    ...
    const message = offer?.benefitType === "PROFESSIONAL_REWARD"
      ? "Reward campaign joined. Complete the milestones to unlock credits."
      : "Coupon claimed successfully";
    return res.status(200).json({ success: true, message, claim });
