...
import { claimCoupon, validateCoupon, normalizeCouponCode, getUserAudience } from "../../services/coupon.service.js";
...
export const validateCouponController = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      message: req.body.bookingId ? "Coupon is valid for this booking" : "Coupon is valid",
    });
  } catch (error) {
...
export const getMyCouponClaims = async (req, res) => {
  try {
    ...
    if (audience !== "customer") return res.status(200).json({ success: true, claims: [] });
