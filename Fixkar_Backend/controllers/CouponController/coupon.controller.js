...
export const claimCouponController = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Coupon claimed successfully", claim });
  } catch (error) {
    return res
      .status(400)
      .json({
        success: false,
        message: error.message || "Unable to claim coupon",
      });
  }
};
  ...
  try {
    ...
    return res.status(200).json({
      ...
      message: req.body.bookingId
        ? "Coupon is valid for this booking"
        : "Coupon is valid",
    });
  } catch (error) {
    return res
      .status(400)
      .json({ success: false, message: error.message || "Invalid coupon" });
  }
};
...
export const applyCouponToBookingController = async (req, res) => {
  try {
    ...
    if (!claim)
      return res
        .status(400)
        .json({ message: "Claim this coupon before applying it to a booking" });
    result.booking.offerId = result.offer._id;
    result.booking.offerCode = result.offer.couponCode;
    result.booking.offerSnapshot = {
      title: result.offer.offerTitle,
...
export const getMyCouponClaims = async (req, res) => {
  try {
    const audience = await getUserAudience(req.userId);
    if (audience !== "customer")
      return res.status(200).json({ success: true, claims: [] });
    const claims = await OfferClaim.find({ userId: req.userId })
      .populate(
        "offerId",
        "couponCode offerTitle description discountType discountValue startDate endDate",
      )
      .sort({ createdAt: -1 });
    return res.status(200).json({ success: true, claims });
  } catch (error) {
