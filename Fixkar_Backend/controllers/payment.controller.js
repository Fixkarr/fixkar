...
export const createOrder = async (req, res) => {
  try {
    ...
    const customer = await Customer.findOne({ userId: req.userId }).select(
      "_id",
    );
    if (!customer)
      return res
        .status(403)
        .json({ message: "Only customers can make booking payments." });
    ...
    if (!booking)
      return res
        .status(404)
        .json({ message: "Booking not found or not available to you" });
    ...
    if (paymentType === "FINAL") {
      if (["cancelled", "rejected", "completed"].includes(booking.status))
        return res
          .status(400)
          .json({
            message: "Final payment is not available for this booking.",
          });
      if (booking.status !== "in-progress")
        return res
          .status(400)
          .json({
            message: "Final payment is available after the service starts.",
          });
      if (!booking.isPriceLocked && !booking.quoteAmount)
        return res.status(400).json({ message: "Quote not sent yet" });
    ...
    } else return res.status(400).json({ message: "Invalid payment type" });
    if (amount <= 0)
...
export const verifyPayment = async (req, res) => {
  ...
  try {
    ...
    if (
      ["cancelled", "rejected", "completed"].includes(customerBooking.status)
    ) {
      await session.abortTransaction();
      session.endSession();
      return res
    ...
    if (!payment) {
      ...
      return res
        .status(400)
        .json({ message: "Payment already processed or invalid" });
    }
    ...
    if (payment.paymentType === "FINAL")
      milestoneResult = await rewardCompletedBookingCredits({
        booking,
        walletId: wallet._id,
    ...
    io.to(booking.professionalId.userId._id.toString()).emit("notification", {
      title: notification.title,
      message: notification.message,
