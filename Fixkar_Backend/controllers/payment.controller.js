...
export const createOrder = async (req, res) => {
  try {
    ...
    if (!customer) return res.status(403).json({ message: "Only customers can make booking payments." });
    const booking = await Booking.findOne({ _id: bookingId, customerId: customer._id }).populate({ path: "professionalId", select: "profession" });
    if (!booking) return res.status(404).json({ message: "Booking not found or not available to you" });
    let discountAmount=0, amount=0, paymentReason="";
    if(paymentType==="FINAL"){
      if(["cancelled","rejected","completed"].includes(booking.status))return res.status(400).json({message:"Final payment is not available for this booking."});
      if(booking.status!=="in-progress")return res.status(400).json({message:"Final payment is available after the service starts."});
      if(!booking.isPriceLocked&&!booking.quoteAmount)return res.status(400).json({message:"Quote not sent yet"});
      const fullAmount=booking.isPriceLocked?booking.totalAmount:booking.quoteAmount+booking.visitingCharge;
      amount=booking.offerLocked&&booking.finalCustomerPayable?booking.finalCustomerPayable:fullAmount;
      discountAmount=booking.offerLocked?booking.discountAmount:0;paymentReason="SERVICE_PAYMENT";
    }else if(paymentType==="CANCEL"){amount=50+(booking.visitingCharge||0);paymentReason="LATE_CANCELLATION_FEE";}else return res.status(400).json({message:"Invalid payment type"});
    if(amount<=0)return res.status(400).json({message:"Invalid payment amount"});
    const alreadyPaid=await Payment.findOne({bookingId,paymentType,status:"paid"});if(alreadyPaid)return res.status(400).json({message:"Payment already completed for this booking"});
    const payment=await Payment.create({bookingId:booking._id,customerId:booking.customerId,professionalId:booking.professionalId,amount,status:"created",reason:paymentReason,paymentType,discountAmount});
...
export const verifyPayment = async (req, res) => {
  ...
  try{
    ...
    if(!customerBooking){await session.abortTransaction();session.endSession();return res.status(403).json({message:"You are not authorized to verify this payment."});}
    if(["cancelled","rejected","completed"].includes(customerBooking.status)){await session.abortTransaction();session.endSession();return res.status(400).json({message:"Payment is not available for this booking."});}
    const body=razorpay_order_id+"|"+razorpay_payment_id;
    const expectedSignature=crypto.createHmac("sha256",process.env.RAZORPAY_API_SECRET).update(body).digest("hex");
    if(expectedSignature!==razorpay_signature){await session.abortTransaction();session.endSession();return res.status(400).json({message:"Invalid payment signature"});}
    const payment=await Payment.findOneAndUpdate({bookingId,razorpayOrderId:razorpay_order_id,status:{$ne:"paid"}},{$set:{status:"paid",razorpayPaymentId:razorpay_payment_id,paidAt:new Date(),paymentMode:"ONLINE"}},{new:true,session});
    ...
    booking.walletTransaction=walletTransaction[0]._id;await booking.save({session});await session.commitTransaction();session.endSession();
