...
const offerSchema = new mongoose.Schema(
  {
    ...
    rewardTrigger: {
      type: String,
      enum: ["FIRST_COMPLETED_BOOKING", "BOOKING_COUNT_MILESTONE", null],
      default: null,
    },
...
offerSchema.pre("validate", function (next) {
  ...
  let audience = Array.isArray(this.audience)
    ? this.audience.filter(Boolean)
    : [];
  if (audience.length !== 1) {
    if (this.benefitType === "CUSTOMER_DISCOUNT") audience = ["customer"];
    else if (this.benefitType === "PROFESSIONAL_REWARD")
  ...
  if (this.benefitType === "CUSTOMER_DISCOUNT") {
    if (audience[0] !== "customer")
      return next(
        new Error("Customer discount coupons are only for customers"),
      );
    if (
      !this.discountType ||
      !Number.isFinite(Number(this.discountValue)) ||
  ...
  if (this.benefitType === "PROFESSIONAL_REWARD") {
    if (audience[0] !== "professional")
      return next(new Error("Professional rewards are only for professionals"));
    ...
    if (
      !["FIRST_COMPLETED_BOOKING", "BOOKING_COUNT_MILESTONE"].includes(trigger)
    )
      return next(new Error("Unsupported professional reward trigger"));

    if (trigger === "FIRST_COMPLETED_BOOKING") {
      const effectiveReward = this.rewardValue ?? this.discountValue;
      if (
        !Number.isFinite(Number(effectiveReward)) ||
        Number(effectiveReward) < 1
      )
        return next(
          new Error("A professional reward of at least 1 credit is required"),
        );
      this.rewardValue = Number(effectiveReward);
      this.milestones = [
        {
          bookingCount: 1,
          rewardCredits: Number(effectiveReward),
          badge: "BRONZE",
          title: "First Booking",
        },
      ];
      ...
      if (!Array.isArray(this.milestones) || this.milestones.length === 0)
        return next(new Error("At least one booking milestone is required"));
      const sorted = [...this.milestones].sort(
        (a, b) => a.bookingCount - b.bookingCount,
      );
      for (let i = 0; i < sorted.length; i += 1) {
        if (i > 0 && sorted[i].bookingCount <= sorted[i - 1].bookingCount)
          return next(
            new Error("Milestone booking counts must be unique and increasing"),
          );
        if (sorted[i].bookingCount < 1 || sorted[i].rewardCredits < 1)
          return next(new Error("Milestone values must be positive"));
      }
  ...
  if (
    ...
    (!Number.isInteger(Number(this.usageLimit)) || Number(this.usageLimit) < 1)
  )
    return next(new Error("Usage limit must be a positive integer or empty"));
  if (
    !Number.isInteger(Number(this.perUserLimit)) ||
    Number(this.perUserLimit) < 1
  )
