...
const offerClaimSchema = new mongoose.Schema(
  {
    ...
    status: {
      type: String,
      enum: ["claimed", "redeemed", "revoked", "expired"],
      default: "claimed",
      index: true,
    },
    bookingId: {
...
offerClaimSchema.index({ offerId: 1, userId: 1 }, { unique: true });
offerClaimSchema.index({ couponCode: 1, status: 1 });

offerClaimSchema.pre("save", function (next) {
