...
export const createOffer = async (req, res) => {
  try {
    ...
    const {
      couponCode, offerTitle, description, audience, benefitType, serviceId = [],
      discountType, discountValue, minBookingAmount, maxDiscount,
      rewardType, rewardValue, startDate, endDate, usageLimit,
      perUserLimit = 1, newCustomerOnly = false, isActive = true,
    } = req.body;
    ...
    if (!code || !offerTitle || !campaignStart || !campaignEnd || audiences.length !== 1) {
      return res.status(400).json({ message: "Coupon code, title, one audience and valid dates are required" });
    }
