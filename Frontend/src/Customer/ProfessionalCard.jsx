...
const ProfessionalCard = ({ data }) => {
  ...
  const rank = rankMeta[data?.achievements?.rank]
    ? data.achievements.rank
    : "NEWCOMER";
  useEffect(() => {
    if (
      !mapsLoaded ||
      ...
      >
        <div className="d-flex align-items-center gap-2">
          ...
          <div className="text-end flex-shrink-0 lh-1">
            ...
            <small style={{ fontSize: "0.68rem", opacity: 0.85 }}>
        ...
        <div className="mt-2">
          <span className="badge rounded-pill bg-light text-dark">
            {rankMeta[rank].icon} {rankMeta[rank].label} Professional
          </span>
        </div>
      </div>
