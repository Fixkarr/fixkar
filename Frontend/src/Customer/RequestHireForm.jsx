...
import { FaUser, FaCalendarAlt, FaClock, FaTools, FaMapMarkerAlt, FaPhoneAlt, FaInfoCircle, FaMicrophone, FaCheck, FaChevronRight, FaTimes, FaRupeeSign, FaMobileAlt } from "react-icons/fa";
...
const RequestHireForm = ({ proInfo, task, onClose }) => {
  ...
  const [taskId, setTaskId] = useState(task?._id || preselectedTask?._id || "");
  ...
  const hasProfessionalVisitingCharge = isDirectHire && proInfo?.isChargesDefined === true && Number.isFinite(Number(proInfo?.visitingCharge));
  const professionalVisitingCharge = hasProfessionalVisitingCharge ? Number(proInfo.visitingCharge) : null;
  const visitingCharge = isDirectHire
    ? (hasProfessionalVisitingCharge ? professionalVisitingCharge : distanceCharge)
    : isSkillBased
      ? distanceCharge
      : null;
  const availableTasks = isDirectHire ? [] : serviceObject?.skills || [];
  const selectedTask = availableTasks.find((item) => item._id?.toString() === taskId?.toString()) || task || preselectedTask;
  ...
  useEffect(() => { if (task) { setTaskId(task._id); setFormData((prev) => ({ ...prev, problemDesc: task.name })); } }, [task]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isMobileVerified) return toast.warning("Please verify your mobile number before hiring a professional.");
  ...
  if (!isMobileVerified) return <div className="booking-experience">
    <div className="booking-header"><div className="booking-header-content"><div className="booking-service-icon"><FaMobileAlt /></div><div className="flex-grow-1"><div className="booking-eyebrow">VERIFICATION REQUIRED</div><h4 className="booking-title">Verify your mobile number</h4><p className="booking-subtitle mb-0">Verify your number before you continue with this service request.</p></div>{onClose && <button type="button" className="booking-close" onClick={onClose} aria-label="Close"><FaTimes size={14}/></button>}</div></div>
    <div className="booking-body">
      <section className="booking-section text-center py-4"><div className="section-icon mx-auto mb-3"><FaPhoneAlt size={15}/></div><h6 className="fw-bold mb-2">Mobile verification is required</h6><p className="text-muted small mb-3">We'll use your verified number to contact you about this service request.</p><button type="button" className="btn btn-primary rounded-pill px-4" onClick={() => navigate("/customer/verify-mobile")}>Verify Mobile Number</button></section>
    </div>
  </div>;

  return <div className="booking-experience">
    <div className="booking-header"><div className="booking-header-content"><div className="booking-service-icon"><FaTools /></div><div className="flex-grow-1"><div className="booking-eyebrow">SERVICE REQUEST</div><h4 className="booking-title">Let's get your work sorted</h4><p className="booking-subtitle mb-0">Just a few details and we'll take it from here.</p></div>{onClose && <button type="button" className="booking-close" onClick={onClose} aria-label="Close"><FaTimes size={14}/></button>}</div><div className="booking-progress"><div className="progress-step active"><span>1</span><small>Details</small></div><div className="progress-line"/><div className="progress-step"><span>2</span><small>Schedule</small></div><div className="progress-line"/><div className="progress-step"><span>3</span><small>Confirm</small></div></div></div>
    <form onSubmit={handleSubmit} className="booking-body">
      <section className="booking-section"><div className="section-heading"><div className="section-icon"><FaTools size={14}/></div><div><h6>What do you need help with?</h6><small>We've selected the service you requested.</small></div></div><div className="selected-service-card"><div className="selected-service-left"><div className="selected-service-symbol"><FaTools/></div><div><small className="text-muted d-block">Service</small><strong>{serviceName}</strong></div></div>{selectedTask && <div className="selected-task-info"><FaCheck className="text-success" size={12}/><div><small className="text-muted d-block">Selected task</small><strong>{selectedTask.name}</strong></div></div>}</div></section>
      {isDirectHire && (hasProfessionalVisitingCharge || hasDistance) && <section className="booking-section"><div className="section-heading"><div className="section-icon"><FaMapMarkerAlt size={13}/></div><div><h6>Visiting charge</h6><small>{hasProfessionalVisitingCharge ? "Set by this professional." : "Calculated from your location to the professional."}</small></div></div><div className="d-flex align-items-center justify-content-between rounded-3 border p-3 bg-light"><div>{!hasProfessionalVisitingCharge && <><small className="text-muted d-block">Distance</small><strong>{distance?.distance?.text}</strong></>}</div><div className="text-end"><small className="text-muted d-block">Visiting charge</small><strong className="text-success fs-5"><FaRupeeSign size={14}/>{visitingCharge}</strong></div></div></section>}
