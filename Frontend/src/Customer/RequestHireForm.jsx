...
const RequestHireForm = ({ proInfo, task, onClose }) => {
  ...
  if (isDirectHire) {
    visitingCharge = isSpecialized ? Number(proInfo?.visitingCharge || 0) : distanceCharge;
  } else {
  ...
  useEffect(() => {
    if (!isDirectHire && task) {
      setTaskId(task._id);
      setFormData((prev) => ({ ...prev, problemDesc: task.name }));
    }
  }, [task, isDirectHire]);

  useEffect(() => {
    if (selectedLocation?.address) {
      setFormData((prev) => ({ ...prev, workAddress: selectedLocation.address }));
    }
  }, [selectedLocation]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isDirectHire && taskId) formDataToSend.append("taskId", taskId);
    ...
    if (isDirectHire && hasDistance) formDataToSend.append("distanceInKm", distanceInKm);
    ...
    if (!isMobileVerified) {
      return toast.warning(
        <div>
          ...
          <p className="small mb-2">Please verify your mobile number to hire a professional.</p>
          <button className="btn btn-sm btn-primary rounded-pill" onClick={() => { toast.dismiss(); navigate("/customer/verify-mobile"); }}>
    ...
    try {
      setLoading(true);
      const result = await axios.post(`${server_url}/api/booking/create-booking`, formDataToSend, { withCredentials: true });
  ...
  if (pickupSearching) {
    return <PickupWaiting pickupSessionId={pickupSessionId} expiresAt={pickupExpiresAt} onConfirmHire={confirmPickupHire} onExpired={clearExpiredPickup} />;
  }

  const serviceName = typeof service === "string"
    ? service
    : service?.name || proInfo?.profession?.name || proInfo?.profession || "Professional service";

  return (
    <div className="booking-experience">
      <div className="booking-header">
        <div className="booking-header-content">
          <div className="booking-service-icon"><FaTools /></div>
          <div className="flex-grow-1">
            <div className="booking-eyebrow">SERVICE REQUEST</div>
            <h4 className="booking-title">Let's get your work sorted</h4>
            <p className="booking-subtitle mb-0">Just a few details and we'll take it from here.</p>
          </div>
          {onClose && <button type="button" className="booking-close" onClick={onClose}><FaTimes size={14} /></button>}
        </div>
        <div className="booking-progress">
          <div className="progress-step active"><span>1</span><small>Details</small></div>
          <div className="progress-line" />
          <div className="progress-step"><span>2</span><small>Schedule</small></div>
          <div className="progress-line" />
          <div className="progress-step"><span>3</span><small>Confirm</small></div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="booking-body">
        <section className="booking-section">
          <div className="section-heading">
            <div className="section-icon"><FaTools size={14} /></div>
            <div><h6>What do you need help with?</h6><small>We've selected the service you requested.</small></div>
          </div>
          <div className="selected-service-card">
            <div className="selected-service-left">
              <div className="selected-service-symbol"><FaTools /></div>
              <div><small className="text-muted d-block">Service</small><strong>{serviceName}</strong></div>
            </div>
            {selectedTask && <div className="selected-task-info"><FaCheck className="text-success" size={12} /><div><small className="text-muted d-block">Selected task</small><strong>{selectedTask.name}</strong></div></div>}
          </div>

          {availableTasks.length > 0 && (
            <div className="mt-3">
              <label className="booking-mini-label">Need to change the task?</label>
              <select className="booking-select" value={taskId} onChange={(event) => setTaskId(event.target.value)} required>
                <option value="">Select the work you need</option>
                {availableTasks.map((availableTask) => <option key={availableTask._id} value={availableTask._id}>{availableTask.name}{availableTask.bookingType === "inspection" ? " (inspection)" : ""}</option>)}
              </select>
              {taskId && selectedTask?.bookingType === "inspection" && <div className="info-hint"><FaInfoCircle /><span>This task needs inspection. The professional will quote after the visit.</span></div>}
            </div>
          )}
        </section>

        {isDirectHire && hasDistance && (
          <section className="booking-section">
            <div className="section-heading">
              <div className="section-icon"><FaMapMarkerAlt size={13} /></div>
              <div><h6>Visiting charge</h6><small>Based on your distance from the professional.</small></div>
            </div>
            <div className="d-flex align-items-center justify-content-between rounded-3 border p-3 bg-light">
              <div><small className="text-muted d-block">Distance</small><strong>{distance?.distance?.text}</strong></div>
              <div className="text-end"><small className="text-muted d-block">Visiting charge</small><strong className="text-success fs-5"><FaRupeeSign size={14} />{visitingCharge}</strong></div>
            </div>
          </section>
        )}

        <section className="booking-section">
          <div className="section-heading"><div className="section-icon"><FaUser size={13} /></div><div><h6>Who should we contact?</h6><small>Tell us the name we should use when we arrive.</small></div></div>
          <div className="friendly-input"><div className="friendly-input-icon"><FaUser /></div><div className="flex-grow-1"><label>Your name</label><input type="text" name="customerName" value={formData.customerName} onChange={handleChange} placeholder="Enter your full name" required /></div></div>
        </section>

        <section className="booking-section">
          <div className="section-heading"><div className="section-icon"><FaCalendarAlt size={13} /></div><div><h6>When should we come?</h6><small>Pick a convenient date and time.</small></div></div>
          <div className="row g-3">
            <div className="col-md-6"><div className="schedule-card"><div className="schedule-card-top"><FaCalendarAlt /><span>Preferred date</span></div><input type="date" name="workDate" value={formData.workDate} min={today} onChange={handleChange} required />{formData.workDate && <small className="schedule-preview">{formatDate(formData.workDate)}</small>}{busyDays?.includes(formData.workDate) && <small className="text-danger d-block mt-2">This date is not available.</small>}</div></div>
            <div className="col-md-6"><div className="schedule-card"><div className="schedule-card-top"><FaClock /><span>Preferred time</span></div><input type="time" name="workTime" min={formData.workDate === today ? minTime : undefined} value={formData.workTime} onChange={handleChange} required /><small className="schedule-help">Choose any future time.</small></div></div>
          </div>
        </section>

        <section className="booking-section">
          <div className="section-heading"><div className="section-icon"><FaInfoCircle size={13} /></div><div><h6>Tell us what you need</h6><small>A few words are enough. This helps the professional understand the work.</small></div></div>
          <div className="description-box"><textarea name="problemDesc" rows="4" value={formData.problemDesc} onChange={handleChange} readOnly={!isDirectHire && selectedTask?.bookingType === "fixed"} placeholder={!isDirectHire && selectedTask?.bookingType === "fixed" ? "Your selected task will be used here." : "For example: The ceiling fan is making noise..."} required />{!(!isDirectHire && selectedTask?.bookingType === "fixed") && <div className="description-tip"><FaInfoCircle size={10} />You can describe the problem in your own words.</div>}</div>
        </section>
