...
.fixkar-search .search-shell{
  ...
  box-shadow:0 14px 45px rgba(31,41,55,.07);
}
...
.fixkar-search .service-grid{
  display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:10px;
}
.fixkar-search .service-scroll-area{overflow:visible}
.fixkar-search .service-card-wrap{position:relative;min-width:0}
.fixkar-search .service-card{
  width:100%;min-height:112px;padding:12px 9px 10px;
  border:1px solid #e8eaf2;border-radius:17px;background:#fff;
  display:flex;flex-direction:column;align-items:center;justify-content:center;
  gap:8px;text-align:center;cursor:pointer;
  transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease,background .18s ease;
}
.fixkar-search .service-card:hover{
  transform:translateY(-2px);border-color:#cfd2f8;
...
.fixkar-search .service-icon{
  width:48px;height:48px;min-width:48px;border-radius:14px;overflow:hidden;
  background:#f3f4f8;box-shadow:inset 0 0 0 1px rgba(0,0,0,.035);
}
.fixkar-search .service-icon img{width:100%;height:100%;object-fit:cover}
.fixkar-search .service-card-content{min-width:0;width:100%}
...
.fixkar-search .service-check{
  ...
  display:flex;align-items:center;justify-content:center;background:#f0f1f5;color:#9299aa;
}
...
.fixkar-search .task-list{
  display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;padding:0;
  max-height:none;overflow:visible;
}
.fixkar-search .task-chip{
  width:100%;min-width:0;min-height:86px;border:1px solid #e7e9f1;background:#fff;
  border-radius:16px;padding:13px;text-align:left;display:flex;align-items:center;
...
.fixkar-search .task-price{
  display:block;color:#111827;background:transparent;border-radius:0;padding:0;
  font-size:17px;line-height:1.1;font-weight:850;letter-spacing:-.02em;
}
...
@media(max-width:991.98px){
  .fixkar-search .service-grid{grid-template-columns:repeat(4,minmax(0,1fr))}
}
@media(max-width:767.98px){
  .fixkar-search .search-shell{padding:11px;border-radius:20px;box-shadow:0 8px 28px rgba(31,41,55,.055)}
  .fixkar-search .service-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}
  .fixkar-search .service-card{min-height:96px;padding:9px 6px;border-radius:14px}
  .fixkar-search .service-icon{width:40px;height:40px;min-width:40px;border-radius:12px}
  .fixkar-search .service-name{font-size:11.5px}
  .fixkar-search .service-card-hint{display:none}
  .fixkar-search .service-check{top:5px;right:5px;width:19px;height:19px}
  .fixkar-search .task-section{padding:12px;border-radius:17px}
  .fixkar-search .task-list{grid-template-columns:1fr;gap:8px}
  .fixkar-search .task-chip{min-height:76px;padding:11px;border-radius:14px}
  .fixkar-search .task-name{font-size:13px}
  .fixkar-search .task-price{font-size:16px}
...
@media(max-width:390px){
  .fixkar-search .service-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
  .fixkar-search .service-card{min-height:88px}
                ...
                {filteredServices.length ? (
                  ...
                  <div className="row g-2">
                    {filteredServices.map((service) => {
                      const active = selectedServiceId === service._id;
                      return (
                        <div className="service-card-wrap" key={service._id}>
                          <button
                            type="button"
                            onClick={() => handleServiceChange(service)}
                            className={`service-card ${
