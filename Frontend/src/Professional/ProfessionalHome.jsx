...
const ProfessionalHome = () => {
  ...
  useEffect(() => {
    if (currentUserData?.user?.busyDays) {
      setShowSelectedDays(currentUserData.user.busyDays.map((date) => { const d = new Date(date); d.setHours(0, 0, 0, 0); return d; }));
    }
  }, [currentUserData]);

  useEffect(() => {
    if (window.Notification) setShowNotificationModal(Notification.permission !== "granted");
  }, []);

  const handleEnableNotifications = async () => {
    try {
      setNotifLoading(true);
      if (Notification.permission === "denied") { alert("Notifications are blocked. Please enable them manually from your browser settings."); return; }
      const permission = await Notification.requestPermission();
      if (permission === "granted") { await generateFCMToken(); setShowNotificationModal(false); }
    } finally { setNotifLoading(false); }
  };

  return <>
    <EnableNotificationModal show={showNotificationModal} onClose={() => setShowNotificationModal(false)} onEnable={handleEnableNotifications} loading={notifLoading} />
    <div className="professional-dashboard min-vh-100">
      <div className="professional-dashboard__hero"><div className="professional-dashboard__hero-inner">
        <div className="d-flex justify-content-between align-items-start gap-3"><div><h5 className="fw-semibold text-white mb-1">Welcome back,</h5><h3 className="fw-bold text-warning mb-1">{userId?.fullName} 👋</h3><small className="text-white opacity-75">Manage availability & earnings</small></div><DashboardNavigator /></div>
        <NeedHelp user="professional" />
        <div className="d-flex flex-wrap gap-2 mt-3"><button className="btn professional-dashboard__busy-btn fw-semibold rounded-pill px-4 py-2" style={{ background: "rgba(255,255,255,0.2)", backdropFilter: "blur(6px)", color: "#fff", border: "1px solid rgba(255,255,255,0.4)" }} data-bs-toggle="modal" data-bs-target="#exampleModal"><SlCalender className="me-2" />Mark Busy Days</button><button onClick={() => navigate("/professional/pickup")} className="btn fw-semibold rounded-pill px-4 py-2" style={{ background: "#fff", color: "#0d6efd" }}><FaBell className="me-2" />Pickup Requests</button></div>
      </div></div>
      ...
      <div className="professional-dashboard__content">
        ...
        <div className={`professional-dashboard__overview-grid ${!hasOverviewSidePanel ? "professional-dashboard__overview-grid--single" : ""}`}><ProfessionalWallet /><div className="professional-dashboard__side-stack">
          ...
          {bankVerificationStatus === "N/A" && <div className="card professional-dashboard__card border-0 rounded-4 p-3 bg-white"><ProfessionalBankDetails /></div>}
      ...
      {loading ? <div className="professional-dashboard__announcements"><ClipLoader size={30} color="blue" /></div> : announcements?.length > 0 && announcements.map((a) => <AnnouncementBanner key={a._id} announcement={a} />)}

      <div className="modal fade" id="exampleModal" tabIndex="-1" aria-hidden="true"><div className="modal-dialog modal-dialog-centered"><div className="modal-content border-0 rounded-4 shadow-lg"><div className="modal-body position-relative p-4"><button type="button" className="btn-close position-absolute top-0 end-0 m-3" data-bs-dismiss="modal" aria-label="Close"></button><Availability /></div></div></div></div>
    </div>
  </>;
};
